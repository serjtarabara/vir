...
function draw() {
  ...
  if (audioPNNR) {
    audioPNNR.setPosition(t.x, t.y, t.z)
  }
...
function CreateSphereSurface(r = 0.35) {
  let vertexList = [];
  let lon = -Math.PI;
  let lat = -Math.PI * 0.5;
  while (lon < Math.PI) {
...
function initializeAudio() {
  audioHTML = document.getElementById('audio');

  audioHTML.addEventListener('play', handlePlay);

  audioHTML.addEventListener('pause', handlePause);
}

function handlePlay() {
  console.log('play');
  if (!audioCNTXT) {
    audioCNTXT = new audioCNTXT();
    audioSRC = audioCNTXT.createMediaElementSource(audioHTML);
    audioPNNR = audioCNTXT.createPanner();
    audioFLTR = audioCNTXT.createBiquadFilter();

    // Connect audio nodes
    audioSRC.connect(audioPNNR);
    audioPNNR.connect(audioFLTR);
    audioFLTR.connect(audioCNTXT.destination);

    // Set filter parameters
    audioFLTR.type = 'lowshelf';
    audioFLTR.Q.value = 2;
    audioFLTR.frequency.value = 760;
    audioFLTR.gain.value = 12;
...
function toggleFilter() {
  ...
  if (filterCheckbox.checked) {
    ...
    audioPNNR.disconnect();
    audioPNNR.connect(audioFLTR);
    audioFLTR.connect(audioCNTXT.destination);
  } else {
    // Disconnect filter when checkbox is unchecked
    audioPNNR.disconnect();
    audioPNNR.connect(audioCNTXT.destination);
  }
}
